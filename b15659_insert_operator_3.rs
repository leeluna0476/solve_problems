...
fn permutation(
    ...
    if permut.len() == permut_len {
        let mut numstack = Vec::<i32>::new();
        let mut opstack = Vec::<Operator>::new();
        ...
        for i in 0..permut_len {
            ...
            match permut[i] {
                Operator::ADD | Operator::SUB => {
                    ...
                    opstack.push(permut[i]);
                },
                Operator::MUL | Operator::DIV => {
                    numstack.push(permut[i].operate(a, numbers[i + 1]));
                },
            }
        }

        for i in 0..opstack.len() {
            let a = numstack[i];
            let b = numstack[i + 1];

            numstack[i + 1] = opstack[i].operate(a, b);
        }
