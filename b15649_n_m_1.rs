...
fn main() {
    ...
    backtrack(&mut in_use, &mut result, (n, m), &mut writer);
}

fn backtrack(in_use: &mut Vec<bool>, result: &mut Vec<usize>, (n, m): (usize, usize), writer: &mut impl Write) {
    if result.len() == m {
        for i in 0..(m - 1) {
    ...
    for i in 1..(n + 1) {
        if in_use[i] == false {
            result.push(i);
            in_use[i] = true;
            backtrack(in_use, result, (n, m), writer);
            result.pop();
            in_use[i] = false;
        }
    }
